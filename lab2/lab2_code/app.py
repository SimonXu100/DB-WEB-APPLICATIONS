...
def resource_by_id(dbname, resource, primary_key):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            #pass
            # get from OH
            # mapping to find_my_primary_key
            ...
            #
            # -- TO IMPLEMENT --
            # mapping to delete_by_primary_key
            r_table = dta.get_rdb_table(resource, dbname)
            key = primary_key.split(_key_delimiter)
            res = r_table.delete_by_key(key)
            res = str(res) + " rows have been deleted."
            rsp = Response(json.dumps(res), status=200, content_type="application/json")
            return rsp
            ...
            #
            # -- TO IMPLEMENT --
            # mapping to update_by_primary_key

            # get the request body information
            inputs = log_and_extract_input(tbls, None)
            # the type of new_values: dict
            new_values = inputs.get("body", None)
            #print("the result of new_values:   ", json.dumps(new_values, indent=2))

            r_table = dta.get_rdb_table(resource, dbname)
            key = primary_key.split(_key_delimiter)
            # new values
            res = r_table.update_by_key(key, new_values)
            print(res, "  rows have been updated")
            res = str(res) + "  rows have been updated"
            rsp = Response(json.dumps(res), status=200, content_type="application/json")
            return rsp
...
def get_resource(dbname, resource_name):
    ...
    try:
        ...
        if request.method == 'GET':
            ...
            #
            # -- TO IMPLEMENT --
            # mapping to insert a table
            r_table = dta.get_rdb_table(resource_name, dbname)
            new_record = context.get("body", None)
            res = r_table.insert(new_record)

            res = str(res) + "  rows haven been inserted"
            res2 = "HTTP: 200 Entry successfully inserted"
            rsp = Response(json.dumps(res+'   '+res2, default=str), status=200, content_type="application/json")
            return rsp
